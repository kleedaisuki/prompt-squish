...
实现可另提供显式的预编译快照接口：调用者选择复用同一份已冻结、已验证和已链接的程序，再以不同参数进行独立展开。此时源码变化只有重新准备快照后才生效；每次展开仍创建全新的参数环境、来源帧及预算计数。普通 `compile` 和 CLI 的独立入口编译不隐式复用旧快照。静态事件字节的缓存不得合并执行身份，亦不得跳过展开或资源检查。
