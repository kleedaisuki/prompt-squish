...
`LinkedImage` is deliberately non-executable metadata. Its addresses and region references become usable only after every referenced semantic unit blob has been loaded and validated and `LinkedProgram` has built its session-local indexes. Public reconstruction validates the image and each unit because its inputs may come from storage; the static linker reuses the validation already performed on its frozen closure and newly constructed image rather than traversing every unit arena twice. `StaticLinkMap` and `LinkedImage` may be persisted; `LinkedProgram` may be memoized only within a process session and has no wire encoding or CAS identity.
