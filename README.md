...
Paths accept files, directories, or quoted globs. No paths prints help. Generated artifacts are excluded from discovery. Independent inputs may continue after an error, but any failure yields a nonzero exit status. Consult `xmlsquish --help` for options.
...
| 结构 / Form | 契约 / Contract |
| --- | --- |
| `xs:module` | 声明容器；其主体是隐式 `main` / Declaration container with implicit `main` body |
...
| `xs:call ref="app:name"` | 静态命名宏调用 / Static named macro call |
| `xs:mount src="..."` | 调用目标模块 `main` / Call a module's `main` |
